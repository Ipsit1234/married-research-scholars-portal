...
class VacatedListAdmin(admin.ModelAdmin):
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == "applicant":
            ...
            id = path.split('/')[-3]
            obj = VacatedList.objects.get(pk=id)
            kwargs['queryset'] = Applicant.objects.filter(Q(occupied_Type1=True) | Q(occupied_Tulsi=True) | Q(occupied_MRSB=True), occupiedlist__id__isnull=False, vacatedlist__id__isnull=False,
                                                          occupiedlist__building=obj.building)
        return super().formfield_for_manytomany(db_field, request, **kwargs)
